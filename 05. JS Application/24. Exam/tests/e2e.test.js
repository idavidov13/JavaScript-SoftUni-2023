...
const endpoints = {
  ...
  own: (itemId, userId) =>
    `/data/cyberpunk?where=_id%3D%22${itemId}%22%20and%20_ownerId%3D%22${userId}%22&count`,
};
...
describe("E2E tests", function () {
  ...
  describe("Navigation bar [ 10 Points ]", () => {
    ...
    it("Guest user should see correct navigation [ 2.5 Points ]", async function () {
      ...
      expect(await page.isVisible("nav"), "Dashboard is not visible").to.be
        .true;
      expect(await page.isVisible("nav >> text=Sell"), "Create is visible").to
        .be.false;
    ...
    it("Guest user navigation should work [ 2.5 Points ]", async function () {
      ...
      let marketBtn = await page.waitForSelector("nav >> text=Market", {
        timeout: interval,
      });
    ...
    it("Logged in user navigation should work [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("#dashboard", { timeout: interval });
      let createBtn = await page.waitForSelector("nav >> text=Sell", {
        timeout: interval,
      });
  ...
  describe("Home Page [ 5 Points ]", function () {
    it("Show Home page text [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector(
        "text=We know who you are, we will contact you",
        { timeout: interval }
      );
      expect(
        await page.isVisible("text=We know who you are, we will contact you")
      ).to.be.true;
    });
  ...
  describe("Dashboard Page [ 15 Points ]", function () {
    ...
    it("Check Items have correct availability [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector(".item-info .availability", {
        timeout: interval,
      });
  ...
  describe("CRUD [ 50 Points ]", () => {
    describe("Create [ 12.5 Points ]", function () {
      it("Create does NOT work with empty fields [ 2.5 Points ]", async function () {
        ...
        let addItemBtn = await page.waitForSelector("text=Sell", {
          timeout: interval,
        });
      ...
      it("Create makes correct API call [ 2.5 Points ]", async function () {
        ...
        let modelElement = await page.waitForSelector('[name="item"]', {
          timeout: interval,
        });
        ...
        let weightElement = await page.waitForSelector(
          '[name="availability"]',
          { timeout: interval }
        );
        let speedElement = await page.waitForSelector('[name="type"]', {
          timeout: interval,
        });
        let aboutElement = await page.waitForSelector('[name="description"]', {
          timeout: interval,
        });
        let submitBtn = await page.waitForSelector('[type="submit"]', {
          timeout: interval,
    ...
    describe("Details [ 10 Points ]", function () {
      it("Details calls the correct API [ 2.5 Points ]", async function () {
        ...
        let moreInfoButton = await page.waitForSelector(
          `.item:has-text("${data.item}") >> .details-btn`,
          { timeout: interval }
        );
      ...
      it("Details with guest calls shows correct info [ 2.5 Points ]", async function () {
        ...
        let priceElement = await page.waitForSelector(".details-price", {
          timeout: interval,
        });
        let weightElement = await page.waitForSelector(
        ...
        let speedElement = await page.waitForSelector(".type", {
          timeout: interval,
        });
        let aboutElement = await page.waitForSelector("#item-description", {
          timeout: interval,
        });
    ...
    describe("Edit [ 17.5 Points ]", function () {
      ...
      it("Edit does NOT work with empty fields [ 2.5 Points ]", async function () {
        ...
        let modelInput = await page.waitForSelector('[name="item"]', {
          timeout: interval,
        });
        let imageInput = await page.waitForSelector('[name="imageUrl"]', {
          timeout: interval,
        });
        let yearInput = await page.waitForSelector('[name="price"]', {
          timeout: interval,
        });
        let mileageInput = await page.waitForSelector('[name="availability"]', {
          timeout: interval,
        });
        let contactInput = await page.waitForSelector('[name="type"]', {
          timeout: interval,
        });
