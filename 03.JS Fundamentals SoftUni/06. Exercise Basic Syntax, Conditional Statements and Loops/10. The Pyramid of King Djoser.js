...
// Note the top-most layer is made entirely out of gold, with a height equal to the given increment. See the examples
...
function pyramid(dimensions, increment) {
  let totalSteps = Math.ceil(dimensions / 2);
  let stone = 0;
  let marble = 0;
  let lapis = 0;
  let gold = 0;
  ...
  for (let i = 1; i <= totalSteps; i++) {
    if (i === totalSteps) {
      ...
      //calculate lapis
      lapis = lapis + (width - 1) * 4 * increment;
      stone = stone + (width - 2) * (width - 2) * increment;
    } else {
      //calculate stones and marbles
      marble = marble + (width - 1) * 4 * increment;
      stone = stone + (width - 2) * (width - 2) * increment;
    }
    width -= 2;
  }
