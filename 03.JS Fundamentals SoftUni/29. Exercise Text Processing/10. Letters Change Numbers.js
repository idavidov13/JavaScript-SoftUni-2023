...
function solve(str) {
  ...
  for (let each of arr) {
    each = each.trim();
    let eachArr = each.split("");
    let firstLetter = eachArr.shift();
    let secondLetter = eachArr.pop();
    //console.log(eachArr);
    let number = Number(eachArr.join(""));

    if (firstLetter.charCodeAt(0) > 64 && firstLetter.charCodeAt(0) < 91) {
      score += number / (firstLetter.charCodeAt(0) - 64);
    } else if (
      firstLetter.charCodeAt(0) > 96 &&
      firstLetter.charCodeAt(0) < 123
    ) {
      score += number * (firstLetter.charCodeAt(0) - 96);
    }

    if (secondLetter.charCodeAt(0) > 64 && secondLetter.charCodeAt(0) < 91) {
      score -= secondLetter.charCodeAt(0) - 64;
    } else if (
      secondLetter.charCodeAt(0) > 96 &&
      secondLetter.charCodeAt(0) < 123
    ) {
      score += secondLetter.charCodeAt(0) - 96;
    }
  }
